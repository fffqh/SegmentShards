...
class SamImgembTargetAdversarialGenerator:
    def __init__(
        self,
        model: Sam,
    ...
    def run(
        ...
        for crop_box, layer_idx in zip(crop_boxes, layer_idxs):
            ...
            im_torch_adv, img_embeddings_adv = self._attack_vit(im_torch, im_torch_target)
            ...
            for (points,) in batch_iterator(self.points_per_batch, points_for_image):
                ...
                sparse_embeddings, dense_embeddings = self.sam.prompt_encoder(
                    points = points_torch_tuple,
                    boxes=None,
                    masks=None
                )
                # 计算decoder输出
                low_res_masks, iou_predictions = self.sam.mask_decoder(
                    image_embeddings=img_embeddings_adv,
                ...
                # 后处理
                masks = self.sam.postprocess_masks(low_res_masks, self.input_size, self.original_size)
                if not self.return_logists:
                ...
                batch_data = self._batchdata_postprocess(batch_data, orig_size, crop_box)
                crop_data.cat(batch_data)
            
            crop_data = self._cropdata_postprocess(crop_data, crop_box)
            mask_data.cat(crop_data)
            
            # 用im_torch更新adv_image
            im_array = self._get_advimage(im_torch_adv)
            adv_image[y0:y1,x0:x1,:] = im_array

        # Remove duplicate masks between crops
        if len(crop_boxes) > 1:
            # Prefer masks from smaller crops
            scores = 1 / box_area(mask_data["crop_boxes"])
            scores = scores.to(mask_data["boxes"].device)
        ...
        mask_data.to_numpy()
        curr_anns = self._maskdata_postprocess(mask_data=mask_data)
        return curr_anns, adv_image
    ...
    def _attack_vit(
        ...
        tg_img_embeddings = self.sam.image_encoder(im_torch_target)
        self.tg_img_embeddings = tg_img_embeddings.detach()
        del tg_img_embeddings

        im_torch = Variable(im_torch_ori, requires_grad=True)
        optimizer = torch.optim.Adam([im_torch])
        loss_function = torch.nn.MSELoss()
        momentum = torch.zeros_like(im_torch).detach().to(im_torch.device)
        ### 开始self.iter轮攻击
        with tqdm(range(self.adv_iter)) as tbar:
            for i in tbar:
                ...
                # 计算攻击损失函数
                loss = loss_function(img_embeddings, self.tg_img_embeddings)
                optimizer.zero_grad()
                loss.backward()
                tbar.set_postfix(epoch=i, loss='{0:1.4f}'.format(loss.data.item()))
                # 计算梯度
        ...
        ### 攻击结束
        im_torch_adv = im_torch.detach()
        img_embeddings_adv = img_embeddings.detach()
        return im_torch_adv, img_embeddings_adv
