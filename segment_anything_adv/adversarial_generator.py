...
class SamAdversarialGenerator:
    ...
    def run(self, image:np.ndarray) -> Tuple[List[Dict[str, Any]], np.ndarray]:
        ...
        for crop_box, layer_idx in zip(crop_boxes, layer_idxs):
            ...
            for (points,) in batch_iterator(self.points_per_batch, points_for_image):
                ...
                ## 进行攻击
                masks, iou_preds, _, adv_img_torch = self._attack(im_torch, im_torch_ori, points_torch_tuple)
                ...
                #print('batch_data :', batch_data._stats)
                batch_data = self._batchdata_postprocess(batch_data, orig_size, crop_box)
                #print('batch_data p:', batch_data._stats)
                crop_data.cat(batch_data)
                #print(crop_data._stats)
    ...
    def _attack(
        ...
        with tqdm(range(self.adv_iter)) as tbar:
            for i in tbar:
                ...
                if self.tg_low_res_masks is None:
                    self.tg_low_res_masks = torch.rand(low_res_masks.shape).to(low_res_masks.device)
                    self.tg_iou_predictions = torch.rand(iou_predictions.shape).to(low_res_masks.device)
        ...
        return masks.detach(), iou_predictions.detach(), low_res_masks.detach(), im_torch.detach()
